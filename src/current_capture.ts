...
export interface CurrentCaptureControls {
  exposure: { paramId: string; mode: 1; value: number };
  gain: { paramId: string; mode: 1; value: number };
  frameCount: { paramId: string; value: number };
}
...
export interface CurrentCaptureTransport {
  ...
  readonly state?: { session: { generation: number } };
}
...
const running = new WeakSet<CurrentCaptureTransport>();
const int32Max = 2_147_483_647;
function positiveInt(value: number, description: string): void {
  if (!Number.isSafeInteger(value) || value < 1 || value > int32Max)
    throw new CurrentProtocolError(
      "invalid-parameter",
      `${description} must be an integer in [1, ${int32Max}]`,
    );
}
function namespace(
  ...
  if (!parameter.paramId)
    throw new CurrentProtocolError(
      "unsupported",
      `No discovered identifier for ${parameter.key}`,
    );
  const parts = decodeCurrentParamId(parameter.paramId);
  if (
    parts.shootingMode !== 2 ||
    ...
    throw new CurrentProtocolError(
      "invalid-parameter",
      `Discovered ${parameter.key} identifier does not belong to this astronomy camera/control`,
    );
  }
...
export function resolveCurrentScienceFilter(
  ...
  const matches = profile.scienceFilters.filter((filter) =>
    typeof value === "number"
      ? filter.index === value
  ...
  if (matches.length !== 1)
    throw new CurrentProtocolError(
      "invalid-parameter",
      "Choose a science filter supported by the connected DWARF model; dark frames use the calibration workflow",
    );
  return matches[0].index;
}

/** All catalog/settings validation occurs before any mode or capture command. */
export function resolveCurrentCaptureControls(
  profile: CurrentProfile,
  catalog: CurrentCameraCatalog,
  settings: CurrentCaptureSettings,
): CurrentCaptureControls {
  if (catalog.modeId !== 2)
    throw new CurrentProtocolError(
      "invalid-parameter",
      "Astronomy capture requires a freshly discovered mode-2 parameter catalog",
    );
  if (settings.cameraId !== 0 && settings.cameraId !== 1)
    throw new CurrentProtocolError(
      "invalid-parameter",
      "Unknown capture camera",
    );
  positiveInt(settings.frameCount, "Frame count");
  if (
    !Number.isSafeInteger(settings.gain) ||
    settings.gain < 0 ||
    settings.gain > int32Max
  )
    throw new CurrentProtocolError(
      "invalid-parameter",
      "Gain must be a non-negative integer",
    );
  if (settings.cameraId === 0) {
    if (settings.filterIndex === undefined)
      throw new CurrentProtocolError(
        "invalid-parameter",
        "Select a telephoto science filter before capture",
      );
    resolveCurrentScienceFilter(profile, settings.filterIndex);
  } else if (settings.filterIndex !== undefined) {
    throw new CurrentProtocolError(
      "invalid-parameter",
      "Wide-angle capture does not accept a telephoto filter selection",
    );
  }
  const exposureParameter = findCurrentCameraParameter(
    catalog,
    settings.cameraId,
    "exp",
  );
  const gainParameter = findCurrentCameraParameter(
    catalog,
    settings.cameraId,
    "gain",
  );
  ...
  const exposure = selectCurrentExposure(
    catalog,
    settings.cameraId,
  ...
  const countParameters =
    catalog.cameras
      ...
      ?.parameters.filter((parameter) => {
        if (!parameter.paramId) return false;
        const parts = decodeCurrentParamId(parameter.paramId);
        return (
          parts.shootingMode === 2 &&
  ...
  if (countParameters.length !== 1)
    throw new CurrentProtocolError(
      "unsupported",
      "No unique runtime astronomy frame-count control was discovered for this camera",
    );
 ...
 */
export function buildCurrentCapturePlan(
  profile: CurrentProfile,
  catalog: CurrentCameraCatalog,
  settings: CurrentCaptureSettings,
  quickSets: Record<string, unknown>,
): CurrentCapturePlan {
  const controls = resolveCurrentCaptureControls(profile, catalog, settings);
  if ((quickSets.code ?? 0) !== 0)
    throw new CurrentProtocolError(
      "device",
      `Quick-set discovery failed (${quickSets.code})`,
      Number(quickSets.code),
      11040,
    );
  if (
    (quickSets.cameraType ?? 0) !== settings.cameraId ||
    !Array.isArray(quickSets.quickSetList)
  )
    throw new CurrentProtocolError(
      "decode",
      "Quick-set response does not match the selected camera",
    );
  const templates = quickSets.quickSetList.flatMap((entry) => {
    if (!entry || typeof entry !== "object" || typeof entry.infoId !== "string")
      return [];
    if (
      entry.cameraType !== undefined &&
    ...
    if (
      ...
      return [];
    return [
      { infoId: entry.infoId as string, parts, exposure, gain, resolution },
    ];
  });
  templates.sort(
    (first, second) =>
      Math.abs(first.exposure - settings.exposureSeconds) -
  ...
  if (!templates.length)
    throw new CurrentProtocolError(
      "unsupported",
      "The camera returned no usable astronomy quick-set template",
    );
 ...
 */
export function currentCaptureControlsForObservedMode(
  controls: CurrentCaptureControls,
  modeId: number,
): CurrentCaptureControls {
  if (modeId !== 11 && modeId !== 13)
    throw new CurrentProtocolError(
      "unsupported",
      "No verified capture-runtime parameter namespace for this observed mode",
    );
  return {
    exposure: {
      ...controls.exposure,
...
export async function executeCurrentCapture(
  transport: CurrentCaptureTransport,
  profile: CurrentProfile,
  catalog: CurrentCameraCatalog,
  settings: CurrentCaptureSettings,
): Promise<CurrentCaptureSubmission> {
  resolveCurrentCaptureControls(profile, catalog, settings);
  if (running.has(transport))
    throw new CurrentProtocolError(
      "busy",
      "Astronomy capture configuration is already in progress",
    );
  ...
  const sameSession = () => {
    if (
      ...
      throw new CurrentProtocolError(
        "transport",
        "Connection changed during astronomy capture configuration",
      );
  };
  const send = async (
    operation: CurrentCommand,
  ...
  try {
    ...
    if (mode.data.shootingModeId !== 8)
      throw new CurrentProtocolError(
        "device",
        "DWARF did not confirm astronomy mode 8",
        undefined,
        16402,
      );
    ...
    if (![2, 8].includes(camera.data.shootingModeId))
      throw new CurrentProtocolError(
        "device",
        "DWARF did not confirm the astronomy camera",
        undefined,
        16404,
      );
    ...
    if (tech.data.shootingTechId !== 2)
      throw new CurrentProtocolError(
        "device",
        "DWARF did not confirm deep-sky stacking technology",
        undefined,
        16403,
      );
    ...
    for (const step of plan.steps) {
      ...
      if (
        ...
        throw new CurrentProtocolError(
          "device",
          "DWARF returned a different astronomy quick-set configuration",
          undefined,
          11041,
        );
    }
