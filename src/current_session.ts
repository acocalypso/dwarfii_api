...
type Pending = {
  ...
  resolve: (packet: CurrentPacket) => void;
...
export class CurrentSession {
  ...
  private listeners = new Set<
    (state: CurrentSessionState, packet?: CurrentPacket) => void
  >();
  private current: CurrentSessionState = {
    phase: "disconnected",
    ownership: "unknown",
  ...
  constructor(public readonly profile: CurrentProfile) {}
  get state(): CurrentSessionState {
    return this.current;
  }

  subscribe(
    listener: (state: CurrentSessionState, packet?: CurrentPacket) => void,
  ): () => void {
  ...
  close(reason = "Transport disconnected"): void {
    this.send = undefined;
  ...
  request(
    ...
    if (!this.send)
      return Promise.reject(
        new CurrentProtocolError("transport", "DWARF transport is not open"),
      );
    ...
    if (this.pending.has(key))
      return Promise.reject(
        new CurrentProtocolError("busy", `Request ${key} is already pending`),
      );
    ...
    if (this.timedOut.has(key))
      return Promise.reject(
        new CurrentProtocolError(
          "timeout",
          `Reconnect before retrying timed-out request ${key}`,
        ),
      );
    ...
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        reject(
          new CurrentProtocolError("timeout", `DWARF request ${key} timed out`),
        );
      }, timeoutMs);
  ...
  receive(bytes: Uint8Array): CurrentPacket {
    ...
    if (!this.send) return packet; // Frames from a disposed session have no authority.
    ...
    if (packet.known && packet.cmd === 15264 && [2, 3].includes(packet.type)) {
      const matches = [...this.pending.entries()].filter(
        ([, candidate]) =>
          ["setExposure", "setGain", "setIntegerParameter"].includes(
            candidate.operation,
          ) &&
      ...
      if (matches.length === 1) requestKey = matches[0][0];
    }
    ...
    const pending =
      packet.type === 1 || packet.type === 3 || requestKey !== key
    ...
    if (!packet.known || packet.type === 0) return packet;
    ...
    if (
      ...
      if (ownership) this.updateOwnership(ownership);
    }
    if (
      packet.type === 2 ||
      ...
      if (packet.cmd === 15223) this.updateOwnership(packet.data);
  ...
  private updateOwnership(status: Record<string, any>): void {
    // Proto3 omitted mode means zero (host), but lock=false is not control.
    const ownership =
      (status.mode ?? 0) === 0 && status.lock === true ? "control" : "slave";
    this.current = { ...this.current, ownership };
  }

  private emit(packet?: CurrentPacket): void {
    for (const listener of this.listeners) listener(this.current, packet);
  }
}
