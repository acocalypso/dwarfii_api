 ...
 */
import { CurrentProtocolError, getCurrentProfile, } from "./current_protocol.js";
 ...
 */
export function parseCurrentJsonLossless(text) {
...
function responseData(input) {
    const response = record(typeof input === "string" ? parseCurrentJsonLossless(input) : input, "DWARF response");
    if (typeof response.code !== "number" ||
        !Number.isSafeInteger(response.code)) {
        throw new CurrentProtocolError("decode", "DWARF response has no valid result code");
    }
    if (response.code !== 0) {
        throw new CurrentProtocolError("device", `DWARF discovery failed (${response.code})`, response.code);
    }
    return record(response.data, "DWARF response data");
}
function byte(value, description) {
    if (typeof value !== "number" ||
        !Number.isInteger(value) ||
        value < 0 ||
        value > 255) {
        throw new CurrentProtocolError("invalid-parameter", `${description} must be an integer in [0, 255]`);
    }
    return value;
}
export function normalizeCurrentParamId(value) {
    ...
    if (typeof text !== "string" ||
        ...
        throw new CurrentProtocolError("invalid-parameter", "Parameter ID must be an unsigned, lossless uint64 decimal string");
    }
    return text;
}
export function decodeCurrentParamId(paramId) {
...
export function encodeCurrentParamId(parts) {
    ...
    if (reserved > 0xffffffffn)
        throw new CurrentProtocolError("invalid-parameter", "Reserved parameter bits exceed uint32");
    return ((BigInt(mode) << 56n) |
        (BigInt(category) << 48n) |
        (reserved << 16n) |
        (BigInt(camera) << 8n) |
 ...
 */
export function withCurrentParamMode(paramId, shootingMode) {
    const value = BigInt(normalizeCurrentParamId(paramId));
    return ((BigInt(byte(shootingMode, "Parameter shooting mode")) << 56n) |
        (value & modeValueMask)).toString();
}
export function sameCurrentParameterAcrossModes(first, second) {
    return ((BigInt(normalizeCurrentParamId(first)) & modeValueMask) ===
        (BigInt(normalizeCurrentParamId(second)) & modeValueMask));
}
export function normalizeCurrentDeviceInfo(input) {
    ...
    if (typeof hardwareId !== "number" || !Number.isSafeInteger(hardwareId)) {
        throw new CurrentProtocolError("decode", "Device discovery supplied no numeric hardware ID");
    }
    const result = {
        hardwareId,
...
function parameterValue(value) {
    if (typeof value === "string" ||
        ...
        return value;
    throw new CurrentProtocolError("decode", "Unsupported camera parameter value");
}
...
export function parseCurrentExposureSeconds(label) {
    const match = label
        .trim()
        .match(/^(\d+(?:\.\d+)?)(?:\s*\/\s*(\d+(?:\.\d+)?))?\s*(?:s|sec)?$/i);
    if (!match)
        return undefined;
    const seconds = Number(match[1]) / (match[2] === undefined ? 1 : Number(match[2]));
    return Number.isFinite(seconds) && seconds > 0 ? seconds : undefined;
}
function normalizeParameter(input, key, source) {
    const data = record(input, `camera parameter ${key}`);
    const name = typeof data.name === "string" && data.name.trim() ? data.name : key;
    const result = {
        key,
    ...
    if (data.currentMode !== undefined) {
        if (typeof data.currentMode !== "number" ||
            !Number.isSafeInteger(data.currentMode)) {
            throw new CurrentProtocolError("decode", `Invalid current mode for ${key}`);
        }
    ...
    if (data.values !== undefined) {
        ...
        result.options = data.values.map((entry) => {
            const item = entry !== null && typeof entry === "object"
...
export function normalizeCurrentCameraCatalog(input, requestedModeId) {
    const modeId = byte(requestedModeId, "Requested shooting mode");
    const data = responseData(input);
    if (data.modeId !== undefined && data.modeId !== modeId)
        throw new CurrentProtocolError("decode", "Camera catalog mode does not match its request");
    if (!Array.isArray(data.cameraParams))
        throw new CurrentProtocolError("decode", "DWARF response has no camera parameter catalog");
    ...
    const cameras = data.cameraParams.map((entry) => {
        ...
        if (seenCameras.has(cameraId))
            throw new CurrentProtocolError("decode", "Duplicate camera parameter catalog");
        seenCameras.add(cameraId);
        const parameters = [];
        if (camera.specialParams !== undefined) {
            for (const [key, value] of Object.entries(record(camera.specialParams, "special camera parameters"))) {
        ...
        if (camera.generalParams !== undefined) {
            if (!Array.isArray(camera.generalParams))
                throw new CurrentProtocolError("decode", "Invalid general camera parameters");
            camera.generalParams.forEach((entry, index) => {
                const parameter = record(entry, "general camera parameter");
                const key = typeof parameter.name === "string" && parameter.name
                    ? parameter.name
                    : `parameter-${index + 1}`;
...
export function findCurrentCameraParameter(catalog, cameraId, key) {
    const matches = catalog.cameras
        .find((camera) => camera.cameraId === cameraId)
        ?.parameters.filter((parameter) => parameter.key === key) ?? [];
    if (matches.length !== 1)
        throw new CurrentProtocolError("unsupported", `No unique ${key} control for camera ${cameraId} in shooting mode ${catalog.modeId}`);
    return matches[0];
}
export function selectCurrentParameterValue(parameter, value) {
    if (parameter.paramId === undefined ||
        !parameter.options?.some((option) => option.value === value)) {
        throw new CurrentProtocolError("unsupported", `Value is not advertised for ${parameter.key}`);
    }
    return { paramId: parameter.paramId, value };
}
export function selectCurrentExposure(catalog, cameraId, seconds) {
    if (!Number.isFinite(seconds) || seconds <= 0)
        throw new CurrentProtocolError("invalid-parameter", "Exposure duration must be positive seconds");
    const parameter = findCurrentCameraParameter(catalog, cameraId, "exp");
    const matches = parameter.options?.filter((option) => option.seconds !== undefined &&
    ...
    if (matches.length !== 1 ||
        ...
        throw new CurrentProtocolError("unsupported", "No exact exposure duration is advertised by this camera and mode");
    }
